...
# Prime Visualization App Deployment Script
# For use with ssh-alibaba command

echo "🚀 Starting Prime Visualization App deployment..."
...
pip install Flask==3.1.2 numpy==2.3.3 matplotlib==3.10.6 scipy==1.16.2 gunicorn==21.2.0

print_status "Configuring matplotlib for headless environment..."
mkdir -p ~/.matplotlib
echo "backend: Agg" > ~/.matplotlib/matplotlibrc

print_status "Creating application files..."

# Create main application file (Spanish version)
cat > app.py << 'EOF'
import numpy as np
import matplotlib
matplotlib.use('Agg')
...
echo "App Service:"
systemctl status prime-visualization --no-pager -l
echo ""
echo "Nginx Status:"
systemctl status nginx --no-pager -l
echo ""
echo "App Logs (last 10 lines):"
journalctl -u prime-visualization -n 10 --no-pager
EOF

chmod +x /usr/local/bin/prime-viz-status

print_success "Deployment completed successfully!"
print_status "Application is running at: http://$(curl -s ifconfig.me):80"
print_status "Local access: http://localhost"
print_status "Use 'prime-viz-status' to check application status"
print_status "Logs available with: journalctl -u prime-visualization -f"

echo ""
echo "🎉 Prime Visualization App is now live!"
echo "📊 Access your application in a web browser"
echo "🔧 Management commands:"
echo "  - Restart app: systemctl restart prime-visualization"
echo "  - View logs: journalctl -u prime-visualization -f"
